<template>
    <div class="flex h-full w-[calc(100%-7rem)]">
        <div
            class="mt-6 h-[90%] w-[70%] overflow-hidden rounded-xl bg-slate-800 shadow-2xl ring-1 ring-white/10 ring-inset"
        >
            <header
            ...
            >
                <div class="my-auto flex h-[2rem] w-[full]">
                    ...
                    <p
                        class="cursor-defaultmt-2 font-mediu my-auto ml-4 text-xl leading-tight tracking-wide text-white select-none"
                    >
                        歷程記錄
                    </p>
                </div>
                    ...
                    >
                        ...
                        <div class="text-md ml-3 h-full w-[80%] py-4 text-white">
                            ...
                            <div class="mt-2 flex h-8 w-full flex-wrap space-x-3">
                                ...
                                <span class="h-5 truncate text-sm">{{ item.FILE_FORMAT }}</span>
                            </div>
                            <p class="w-full truncate text-slate-300">
                                {{
                                    `下載時間：${timeFormat(item.DOWNLOAD_TIME)}, ${new Date(
                                        item.DOWNLOAD_TIME
                                    ).toLocaleString()}`
                                }}
                            </p>
        ...
        <div
            class="mx-4 mt-6 h-[90%] w-[28%] rounded-lg bg-slate-800 ring-1 ring-white/10 ring-inset"
        >
            <p
                class="my-5 text-center text-3xl leading-tight font-extrabold text-blue-400 select-none"
            >
                控制項
            </p>
            <div class="h-20 w-full">
                <div
                    class="m-auto flex w-[90%] items-center rounded-lg bg-slate-600 p-4 shadow-2xl ring-1 ring-white/10 ring-inset"
                >
                    <div
                        class="flex h-10 w-10 cursor-pointer rounded-full bg-white/30 hover:bg-white/20"
                        @click="clearHistory"
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                    ...
                    <p class="ml-4 w-26 text-lg font-extrabold text-white select-none">
                        清除歷史紀錄
                    </p>
                </div>

                <div
                    class="m-auto mt-5 flex w-[90%] items-center rounded-lg bg-slate-600 p-4 shadow-2xl ring-1 ring-white/10 ring-inset"
                >
                    <div
                        class="flex h-10 w-10 cursor-pointer rounded-full bg-white/30 hover:bg-white/20"
                        @click="toggleChronological"
                    >
                        <svg
                            v-if="chronologicalOldToNew"
                            xmlns="http://www.w3.org/2000/svg"
                            class="m-auto h-6 w-6 fill-white"
                            viewBox="0 0 576 512"
                        >
                    ...
                    <p class="ml-4 w-26 text-lg font-extrabold text-white select-none">
                        {{ chronologicalOldToNew ? '依時間由舊至新' : '依時間由新至舊' }}
                    </p>
                </div>
            </div>
        </div>
...
// Local state
const chronologicalOldToNew = ref(false);
...
function timeFormat(timestamp: number): string {
    ...
    const labels = ['年', '個月', '週', '天', '小時', '分鐘', '秒'];
    const values = [31536000, 2592000, 604800, 86400, 3600, 60, 1];

    for (let i = 0; i < 7; i++) {
        const count = Math.floor(mistiming / values[i]!);
        if (count !== 0) {
            return count + labels[i]! + '前';
        }
    }
    return '剛剛';
}
...
function toggleChronological(): void {
    chronologicalOldToNew.value = !chronologicalOldToNew.value;
    try {
        if (store.historyList) {
            store.setHistoryList([...store.historyList].reverse());
        }
    } catch {
        // ignore
    }
