<template>
    <div
        class="relative h-screen w-screen overflow-hidden rounded-xl bg-slate-900/90 pt-10 ring-1 ring-white/10 ring-inset"
    >
        <div
            data-tauri-drag-region
            class="absolute top-0 left-0 z-50 h-8 w-[calc(100%-7rem)]"
        ></div>

        <!-- window controls wrapper -->
        <div
            class="absolute top-0 z-50 mb-3 flex h-8 w-28 rounded-xl bg-slate-700 transition-all duration-300"
            :class="store.windowControlOnTheLeft ? 'left-0' : 'right-0'"
        >
            <!-- Window minimize -->
            <svg
                class="cursor-pointerh-6 mx-auto my-auto h-6 w-6 fill-slate-200 hover:fill-slate-50"
                viewBox="0 0 24 24"
            ...
            <!-- Window maximize -->
            <svg
                class="mx-auto my-auto h-6 w-6 cursor-pointer stroke-slate-200 hover:stroke-slate-50"
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg"
            ...
            <!-- Window close -->
            <svg
                class="mx-auto my-auto h-6 w-6 cursor-pointer fill-slate-200 hover:fill-slate-50"
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg"
                @click="quit"
            >
        ...
        <!-- 通知 -->
        <notifications group="foo-css" position="bottom right" type="success" :speed="500" />
    </div>
</template>
...
onMounted(async () => {
    ...
    // 載入資料
    await store.fetchHistoryList().catch(console.error);
